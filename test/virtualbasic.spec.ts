...
import { _SERVICE as TestService, idlFactory as TestIdlFactory, init } from './build/virtualbasic.idl.js';
...
const WASM_PATH = resolve(__dirname, "./build/virtualbasic.wasm");
...
describe('Counter', () => {
    ...
    it(`feed ledger user and check if it made the transactions`, async () => {
      ...
      const result = await ledger.icrc1_transfer({
        ...
        from_subaccount: [],
        amount: 1000000_0000_0000n + 1_000n, // virtual version needs one more fee to produce the same results
        fee: [],
        memo: [],
        created_at_time: [],
      });
